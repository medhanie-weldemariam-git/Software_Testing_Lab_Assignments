...
public class CelsiusConverter extends JPanel {
    ...
    public static String fahrenheitOutput(double value, int precision) {
    	// Checking if the temperature is below absolute zero
        if (value < -459.67)
            
        	return "Fahrenheit: NA";
        else
    ...
    private void updateLabels() {
        ...
        try {
            ...
            // Checking if the temperature is below absolute zero
            if (celsius < -273.15){
            	celsiusLabel.setText("Celsius: NA");
            	fahrLabel.setText("Fahrenheit: NA");
            } else {
        ...
        ItemListener listener = new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.SELECTED) {
                    precision = Integer.parseInt(((JRadioButtonMenuItem)e.getSource()).getActionCommand());
                    updateLabels();
                }
            }
